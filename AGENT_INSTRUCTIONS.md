# Neural PagedAttention — Complete Implementation Instructions for Coding Agent

## CRITICAL RULES BEFORE YOU START

1. **Do not make design decisions.** Every design decision is already made in this document. If something is ambiguous, use the default specified. Do not ask — implement.
2. **Do not install real GPU libraries.** No CUDA, no PyTorch tensors, no actual KV cache. Pure Python simulation only.
3. **Implement in the exact order given.** Each section depends on the previous one.
4. **All numbers in this document are final.** Do not change thresholds, multipliers, or constants.

---

## PROJECT STRUCTURE

Create exactly this file structure. Do not add or remove files.

```
neural-paged-attention/
├── server/
...
├── inference.py             # Baseline LLM inference script (root level)
...
version: "1.0.0"
description: >
  A high-fidelity RL environment simulating GPU KV Cache management
  for LLM inference servers. The agent learns to schedule, evict,
  swap, and preempt requests to maximise throughput while honouring
  VIP SLA constraints under chaotic traffic.
author: "Team Neural PagedAttention"
tasks:
  - easy
  - medium
  - hard
...
## STEP 4: server/environment.py

This is the core file. Implement it in this exact order: data structures first, then mechanics, then the public API.

### 4.1 — Constants (top of file, never change these)
...
**This is the most important file for the hackathon automated validator.**
...
Write a README with these exact sections. Fill in baseline scores after running inference.py.
...
[Include the 20-field table from the spec here]

## Action Space
Discrete(18), indices 0–17.
[Include the 18-action table from the spec here]
...
Run these in order. Fix each before moving to the next.

```bash
# 1. Unit test: does the environment run without crashing?
python -c "
from server.environment import KVCacheEnvironment
env = KVCacheEnvironment()
obs = env.reset('easy')
print('Reset OK. Obs:', obs.gpu_utilization_pct)
for i in range(100):
    obs, r, done, info = env.step(17)  # Do Nothing x100
    if done: break
print('100 ticks OK. Score:', env._compute_final_score())
"
...
for task in ['easy', 'medium', 'hard']:
    env.reset(task)
    for _ in range(50):
        obs, r, done, info = env.step(8)  # Admit Free
        if done: break
    print(f'{task}: score={env._compute_final_score():.3f}')
"

# 3. Test FastAPI server
uvicorn server.app:app --host 0.0.0.0 --port 7860 &
sleep 2
curl -s -X POST http://localhost:7860/reset -H 'Content-Type: application/json' -d '{"task":"easy"}' | python -m json.tool
curl -s -X POST http://localhost:7860/step  -H 'Content-Type: application/json' -d '{"action_id":17}' | python -m json.tool
curl -s http://localhost:7860/state | python -m json.tool

# 4. Docker build
docker build -t neural-paged-attention ./server
docker run -d -p 7860:7860 neural-paged-attention
sleep 5
curl -s -X POST http://localhost:7860/reset -H 'Content-Type: application/json' -d '{}' | python -m json.tool

# 5. Run inference (requires HF_TOKEN)
python inference.py
```

---

## KNOWN EDGE CASES — Handle These Explicitly

1. **Empty queues**: If `admit_next` is called on an empty queue, return 0.0 reward and log it. Do not crash.
2. **No eviction targets**: If evict/swap is called but no idle caches exist, return 0.0. Do not crash.
3. **Token cap**: In `generate_request`, cap total tokens at 80% of GPU capacity (already in code above). This prevents single requests that are physically impossible to serve.
4. **Observation when queues empty**: All queue/cache stats default to 0.0 when empty. Use `default=0` in all `max()` calls.
5. **Final score before any completions**: `_compute_final_score()` returns 0.0 if no requests completed. This is correct — don't special-case it.
6. **action_id out of range**: The FastAPI endpoint validates 0-17. The environment's `_execute_action` defaults to no-op for unknown IDs.
